...
while running:
    ...
    motion = position(ro, 5, 250)
    for j in position(ro, 50, 250):
        pygame.draw.circle(window, 'white', [j[0] * cos(j[1]) + 500, j[0] * sin(j[1]) + 500], 1)
    for i in motion:
        pygame.draw.circle(window, 'white', [i[0] * cos(i[1]) + 500, i[0] * sin(i[1]) + 500], 1)
        pygame.draw.line(window, 'white', [500, 500], [i[0] * cos(i[1]) + 500, i[0] * sin(i[1]) + 500], 1)
